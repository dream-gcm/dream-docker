...
For that you can switch to the dedicated notebook: browse the directory tree on the left of the browser page. At the root you'll see `DREAM`and `dream-tools`. In the later, and then in `demo-py-notebooks` you can open the notebook named `2025-04-09_PLT-perturbation-run_DOCKER.ipynb` by double-clicking on it. Then follwo the instructions!
